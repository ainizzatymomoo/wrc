...
# Lazy imports - only import when needed (heavy deps may not be installed)
_IMPORTS_CACHED = {}

def _get_parser():
    if "PDFParser" not in _IMPORTS_CACHED:
        from src.pdf_parser import PDFParser
        _IMPORTS_CACHED["PDFParser"] = PDFParser
    return _IMPORTS_CACHED["PDFParser"]

def _get_mapper():
    if "FieldMapper" not in _IMPORTS_CACHED:
        from src.field_mapper import FieldMapper
        _IMPORTS_CACHED["FieldMapper"] = FieldMapper
    return _IMPORTS_CACHED["FieldMapper"]

def _get_orchestrator():
    if "BLESS2Orchestrator" not in _IMPORTS_CACHED:
        from src.orchestrator import BLESS2Orchestrator
        _IMPORTS_CACHED["BLESS2Orchestrator"] = BLESS2Orchestrator
    return _IMPORTS_CACHED["BLESS2Orchestrator"]
...
async def parse_pdfs(session_id: str):
    ...
    try:
        parser = _get_parser()(ocr_enabled=False)
